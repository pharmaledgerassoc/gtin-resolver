...
class XmlDisplayService {
  ...
  getAvailableLanguagesForBatch(callback) {
    this.getAvailableLanguagesFromPath(this.gtinSSI, this.getBatchPathToXmlType(), callback)
  }

  getAvailableLanguagesForProduct(callback) {
    let gtinSSI = this.getGtinSSIForConstProductDSU();
    this.getAvailableLanguagesFromPath(gtinSSI, this.getProductPathToXmlType(), callback);
  }

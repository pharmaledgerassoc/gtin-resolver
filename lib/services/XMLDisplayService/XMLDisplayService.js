...
class XmlDisplayService {
    ...
    getProductMarketPathToXmlType() {
        return `${constants.PRODUCT_DSU_MOUNT_POINT}/${constants.EPI_MOUNT_PREFIX}/${this.xmlType}`;
    }
    ...
    getAvailableMarketsFromPath(gtinSSI, path, callback) {
        const resolver = openDSU.loadAPI("resolver");
        resolver.loadDSU(gtinSSI, (err, dsu) => {
            if (err) {
                err.isDSUError = err;
            ...
            this.readMarketsFromDSU(dsu, path, callback);
        });
    }

    readMarketsFromDSU(dsu, path, callback) {
        const pskPath = require("swarmutils").path;
        dsu.listFolders(path, async (err, langFolders) => {
            if (err) {
                err.isDSUError = err;

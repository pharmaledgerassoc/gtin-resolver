...
class XmlDisplayService {
  ...
  buildLeafletHTMLSections(resultDocument) {
    let sectionsElements = resultDocument.children[0];
    let htmlContent = "";
    for (let i = 0; i < sectionsElements.children.length; i++) {
      htmlContent = htmlContent + sectionsElements.children[i].outerHTML;
    }
    return htmlContent;
  }

...
const xslContent = require("./leafletXSL");
...
class XmlDisplayService {
  ...
  getAvailableLanguagesFromPath(path, callback) {
    const openDSU = require("opendsu");
    const resolver = openDSU.loadAPI("resolver");
  ...
  getAvailableLanguagesForBatch(callback){
    this.getAvailableLanguagesFromPath(this.getBatchPathToXmlType(), callback)
  }

  getAvailableLanguagesForProduct(callback){
    this.getAvailableLanguagesFromPath(this.getProductPathToXmlType(), callback);
  }

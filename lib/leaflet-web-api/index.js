      ...
      leafletXmlService.readXmlFile(lang, async (err, xmlContent, pathBase, leafletImagesObj) => {
        if (err) {
          ...
          if (getProductData) {
            ...
            let dateStatus = "valid_date";
            try {
              let batchData = await leafletInfo.getBatchClientModel();
              if (batchData.incorrectDateCheck && !getExpiryTime(expiry)) {
                dateStatus = "incorrect_date"
              } else if (batchData.expiredDateCheck && getExpiredStatus(expiry)) {
                dateStatus = "expired_date";
              }
            } catch (e) {
              // gtin only case date is not verified
            }

        ...
        if(knownParams.indexOf(param) === -1){
          ...
          return server.makeLocalRequest("GET", `/leaflets/${domainName}?gtin=${gtin}&lang=${lang}&leaflet_type=${leaflet_type}&batch=${batchNumber}`, (err, content)=>{
            if(err){
              ...
              logger.error(0x100, err);
              logger.error(0x100, err.stack);
              logger.error(0x100, content);
      ...
      try{
        ...
        if(batchExists){
          ...
          if(batchLanguages.indexOf(lang) !== -1){
            ...
            let productLanguages = await $$.promisify(leafletXmlService.getAvailableLanguagesFromPath, leafletXmlService)(prodGTINSSI, leafletXmlService.getProductPathToXmlType());
            if(productLanguages.indexOf(lang) !== -1){
              ...
              if(batchLanguages.length && productLanguages.length){
                preventProductFallback = true;
              }
            }
          }

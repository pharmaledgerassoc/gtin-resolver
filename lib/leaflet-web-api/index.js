  ...
  async function getLeafletHandler(request, response) {
    ...
    try {
      ...
      try{
        ...
        if(batchExists){
          let batchLanguages = await $$.promisify(leafletXmlService.getAvailableLanguagesFromPath)(batchGtinSSI, leafletXmlService.getBatchPathToXmlType());
          if(batchLanguages.indexOf("lang")){
            preventProductFallback = true;
          }

          let prodGTINSSI = GTIN_SSI.createGTIN_SSI(domainName, undefined, gtin);
          let productLanguages = await $$.promisify(leafletXmlService.getAvailableLanguagesFromPath)(prodGTINSSI, leafletXmlService.getBatchPathToXmlType());
          if(productLanguages.indexOf("lang")){
      ...
      if(!batchExists || !preventProductFallback){
        let content;
        try{
          content = await $$.promisify(server.makeLocalRequest)("GET", `/leaflets/${domainName}?gtin=${gtin}&lang=${lang}&leaflet_type=${leaflet_type}`);
        }catch(err){
          if(!preventProductFallback && err.httpCode === 404){
            //this means that the product doesn't the searched language, so we should fall back to dsu handling strategy
          }else{
            logger.error(0x100, err.message);
            return sendResponse(response, 529, `Server busy reading gtin only leaflet` );
          }

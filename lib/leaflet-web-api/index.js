...
function getWebLeaflet(server) {
  ...
  async function getLeafletHandler(request, response) {
    ...
    let epiMarket = request.query.epiMarket || null;
    ...
    try {
      ...
      let knownParams = ["leaflet_type", "gtin", "lang", "batch", "fixedurlrequest", "epiMarket"];
      ...
      if(!batchExists || (batchNumber && !preventProductFallback) ){
        ...
        if(content){
          ...
          if (!content?.availableMarkets) {
            content.productData.availableMarkets = {"en": ["US"], "et": ["AX"]}
          }

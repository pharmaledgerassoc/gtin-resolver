...
function getGTINOwner(server) {
  ...
  const database = createLokiEnclaveFacadeInstance(path.join(cachePath, CACHE_FILE), DATABASE_PERSISTENCE_TIMEOUT, lokiEnclaveFacadeModule.Adaptors.FS);
  ...
  server.get("/gtinOwner/:epiDomain/:gtin/:subdomain", getGtinOwnerHandler);
}

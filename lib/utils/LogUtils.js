...
let instance;
const dbMessageFields = ["pk", "meta", "did", "__timestamp", "$loki", "context", "keySSI"];
...
function MappingLogService(storageService, logService) {
  ...
  this.logSuccessAction = async (message, data, alreadyExists, status, diffs, DSU) => {
    ...
    logData.auditLogData.anchorId = keySSIObj.getAnchorId();
  ...
  this.logFailAction = async (message, data, status) => {
    ...
    logData.auditLogData.reason = logData.auditLogData.reason + " - fail";
    await logAction(logData);
    return logData;
  }
  ...
  this.getDiffsForAudit = (data, prevData) => {
    if (prevData && Object.keys(prevData).length > 0) {
      ...
      let diffs = Object.keys(data).reduce((diffs, key) => {
        ...
        return {
          ...diffs,
          [key]: {oldValue: prevData[key], newValue: data[key]}
        }
      }, {})
      return diffs;
    }
  ...
  let getLogData = (message, data, alreadyExists) => {
    ...
    try {
      switch (message.messageType) {
        ...
        case constants.MESSAGE_TYPES.SMPC:
          ...
          resultObj.itemCode = (message.productCode ? message.productCode : message.batchCode) || resultObj.itemCode;
          resultObj.itemType = message.messageType;
          resultObj.metadata = {
            attachedTo: message.batchCode ? "BATCH" : "PRODUCT",
            itemCode: message.batchCode || message.productCode
          }
        ...
        case constants.MESSAGE_TYPES.VIDEO_SOURCE:
          ...
          resultObj.itemCode = (message.videos.batch ? message.videos.batch : message.videos.productCode) || resultObj.itemCode;
          resultObj.itemType = message.messageType;
      ...
      resultObj.reason = "Failed action";
    ...
    resultObj.auditLogData.logInfo = logInfo;
    return resultObj;
  }
}

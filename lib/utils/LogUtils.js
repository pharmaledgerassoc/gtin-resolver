...
/*  let logMapping = async (message, action, status) => {
    ...
    try {
      await $$.promisify(this.storageService.addIndex.bind(this.storageService))(constants.IMPORT_LOGS, "__timestamp");
      await $$.promisify(this.storageService.insertRecord.bind(this.storageService))(constants.IMPORT_LOGS, logData.itemCode + "|" + currentDate, logData);
...
/*this.logAndUpdateDb = async (message, data, alreadyExists) => {
  let dbTable, pk, metadata = {};
  let logObj = getLogObj(message, data, alreadyExists);
  if (type !== constants.LOG_TYPES.LEAFLET_LOG) {
    await createOrUpdateRecord(this.storageService, logObj.table, logObj.pk, data);
  }

  await this.logSuccessMapping(message, logObj);
}*/

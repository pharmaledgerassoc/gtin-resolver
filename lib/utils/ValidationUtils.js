...
const featManager = require("../DSUFabricFeatureManager");
errorUtils.addMappingError("MVP1_RESTRICTED");

const itemValidator = function (messageValue, schemaObj, schemaKey) {

    if ((typeof messageValue === "string") && messageValue.trim() && !validateForHTMLTags(messageValue)) {
        invalidFields.push({field: schemaKey, message: `HTML tags are not allowed`});
        return;
    }

    if (!schemaObj.required && !messageValue && (schemaObj.type !== "array" || schemaObj.type !== "object")) {
        return;
    }

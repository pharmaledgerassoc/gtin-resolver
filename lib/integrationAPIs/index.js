...
module.exports = function (server) {
    ...
    server.use("/integration/*", async function (req, res, next) {
        ...
        const enclaveInstance = await lightDBEnclaveFactory.createLightDBEnclaveAsync(domain, subdomain, appName);
...
//    server.delete("/integration/epi/:gtin/:batchNumber/:language/:epiType", basicPreValidationMiddleware);
    ...
    server.put("/doDemiurgeMigration", async (req, res) => {
          ...
          const enclaveName = `DB_${process.env.EPI_DOMAIN}_${process.env.EPI_SUBDOMAIN}_Demiurge`;
        // TODO add migration function
        const secretsServiceInstance = await API_HUB.getSecretsServiceInstanceAsync(server.rootFolder);
        await secretsServiceInstance.putSecretInDefaultContainerAsync(enclaveName, crypto.generateRandom(32).toString("base64"));

...
const monsterController = require("./controllers/MonsterController.js").getInstance(version);

const {requestBodyJSONMiddleware} = require("./utils/middlewares.js");
const storage = require("./utils/LightDBEnclaveFactory.js");
...
module.exports = function (server) {
    ...
    server.use("/integration/:domain/*", async function (req, res, next) {
        const {domain} = req.params;
        const enclaveInstance = await storage.getEnclaveInstance(domain, subdomain);
    ...
    async function deleteLeafletHandler(req, res) {
        const {gtin, domain, batchNumber} = req.params;
        ...
        try {
            await leafletController.deleteEPI(domain, gtin, batchNumber, leafletMessage, res);
        } catch (err) {
            res.send(500);
            return;
        }
    ...
    server.delete("/integration/:domain/epi/:gtin", requestBodyJSONMiddleware);
    server.delete("/integration/:domain/epi/:gtin", basicPreValidationMiddleware);
    server.delete("/integration/:domain/epi/:gtin", deleteLeafletHandler);

    server.delete("/integration/:domain/epi/:gtin/:batchNumber", requestBodyJSONMiddleware);
    server.delete("/integration/:domain/epi/:gtin/:batchNumber", basicPreValidationMiddleware);
    server.delete("/integration/:domain/epi/:gtin/:batchNumber", deleteLeafletHandler);

    //------ Messages
    server.put("/integration/:domain/message", requestBodyJSONMiddleware);
    server.put("/integration/:domain/message", async function (req, res) {
        ...
        try {
            await monsterController.digestMessage(domain, message, res);
        } catch (err) {
            res.send(500);
            return;
        }
    ...
    server.put("/integration/:domain/multipleMessages", requestBodyJSONMiddleware);
    server.put("/integration/:domain/multipleMessages", async function (req, res) {
        let message = req.body;
        let domain = req.params.domain;
        ...
        try {
            await monsterController.digestMultipleMessages(domain, message, res);
        } catch (err) {
            res.send(500);
            return;
        }
    ...
    server.put("/integration/:domain/groupedMessages", requestBodyJSONMiddleware);
    server.put("/integration/:domain/groupedMessages", async function (req, res) {
        let message = req.body;
        let domain = req.params.domain;
        ...
        try {
            await monsterController.digestGroupedMessages(domain, message, res);
        } catch (err) {
            res.send(500);
            return;
        }

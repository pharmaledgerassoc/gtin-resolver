const version = 1;
const {requestBodyJSONMiddleware, getIntegrationAPIsAuthorizationMiddleware, getRequestLimiterMiddleware} = require("./utils/middlewares.js");
...
module.exports = function (server) {
    ...
    server.get("/integration/image/:gtin", async function (req, res) {
        ...
        if (req.query && req.query.version) {
            ...
            try{
                ...
                res.send(422, JSON.stringify({message: "Validation failed", details:"Found version query param and is not a integer."}));
                return;
            }
            if(dsuVersion <= 0 || !valid){
                res.send(422, JSON.stringify({message: "Validation failed", details:"Query version param need to be an integer greater then zero."}));
                return;
            }
    ...
    async function productEpiHandler(req, res) {
        ...
        try {
            ...
            if (leafletMessage.payload.productCode !== gtin){
                res.send(422, JSON.stringify({message: "Payload validation failed", details:"Different gtin code between payload and url param"}));
                return;
            }

            if(leafletMessage.payload.batchNumber){
                res.send(422, JSON.stringify({message: "Payload validation failed", details:"Found batch info in payload, when it shouldn't"}));
                return;
            }

            if(language !== leafletMessage.payload.language) {
                res.send(422, JSON.stringify({message: "Payload validation failed", details:"Different language code between payload and url param"}));
                return;
            }
        } catch (err) {
            res.send(415, err.message);
    ...
    async function batchEpiHandler(req, res) {
        ...
        try {
            ...
            if(batchNumber !== leafletMessage.payload.batchNumber){
                res.send(422, JSON.stringify({message: "Payload validation failed", details:"Different batch info between payload and url param"}));
                return;
            }
            if(language !== leafletMessage.payload.language) {
                res.send(422, JSON.stringify({message: "Payload validation failed", details:"Different language code between payload and url param"}));
    ...
    async function getEpiHandler(req, res) {
        let {gtin, batchNumber, language, epiType} = req.params;
        const {domain, subdomain} = getDomainAndSubdomain(req);
        let dsuVersion;
        if (req.query && req.query.version) {
            let valid = false;
        ...
        if (["smpc", "leaflet"].indexOf(epiType) === -1) {
            res.send(400);
            return;
        }
        ...
        try {
            ...
            EPI = await req.leafletController.getEPI(domain, subdomain, gtin, batchNumber, language, epiType, dsuVersion, req, res);
...
//    server.delete("/integration/epi/:gtin/:batchNumber/:language/:epiType", basicPreValidationMiddleware);
    ...
    server.post("/integration/audit/:logType", async (req, res) => {
        ...
        try {
            await req.auditService.addLog(logType, auditMessage, req, res);
        } catch (err) {
    ...
    server.delete("/resetDID/:did", async (req, res) => {
        ...
        try {
            ...
            const constSSI = keySSISpace.createConstSSI(didDocument.getDomain(), didDocument.getName());
            const anchorId = constSSI.getAnchorIdSync();
            const anchorPath = path.join(server.rootFolder, "external-volume", "domains", didDocument.getDomain(), "anchors", anchorId);
    ...
    server.get("/getEpiGroup", async (req, res) => {

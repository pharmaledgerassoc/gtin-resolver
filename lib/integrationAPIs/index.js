...
module.exports = function (server) {
    ...
    async function getEpiHandler(req, res) {
        const {domain, subdomain, gtin, batchNumber, language, type} = req.params;
        ...
        let EPI;
        try {
            EPI = await req.leafletController.getEPI(domain, subdomain, gtin, batchNumber, language, type, res);
        } catch (err) {
            return res.send(500);
        }
        if (!EPI) {
            return res.send(404, `Not found EPI type ${type} for language ${language}`)
        }
        res.setHeader("Content-type", "text/json");
    ...
    server.get("/integration/:domain/:subdomain/epi/:gtin/:language/:type", getEpiHandler);
    server.get("/integration/:domain/:subdomain/epi/:gtin/:batchNumber/:language/:type", getEpiHandler);
    ...
    server.get("/integration/:domain/:subdomain/listProductLangs/:gtin/:epiType", async (req, res) => {
        ...
        let languages;
        try {
            languages = await req.productController.listLanguages(domain, subdomain, gtin, epiType, res);
        } catch (e) {
            return res.send(500);
        }
        res.setHeader("Content-type", "text/json");
    ...
    server.get("/integration/:domain/:subdomain/listBatchLangs/:gtin/:batchNumber/:epiType", async (req, res) => {
        ...
        let languages;
        try {
            languages = await req.batchController.listLanguages(domain, subdomain, gtin, batchNumber, epiType, res);
        } catch (e) {
            return res.send(500);
        }
        res.setHeader("Content-type", "text/json");
    ...
    server.put("/integration/:domain/:subdomain/productGroupedMessages/:gtin", async function (req, res) {
        ...
        try {
            await req.monsterController.digestProductGroupedMessages(domain, subdomain, gtin, messages, res);
        } catch (err) {
            res.send(500);
            return;
        }
    ...
    server.put("/integration/:domain/:subdomain/batchGroupedMessages/:gtin/:batchNumber", async function (req, res) {
        ...
        try {
            await req.monsterController.digestBatchGroupedMessages(domain, subdomain, gtin, batchNumber, messages, res);
        } catch (err) {
            res.send(500);
            return;
        }

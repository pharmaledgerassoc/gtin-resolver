...
module.exports = function (server) {
    ...
    server.post("/integration/product/:gtin", async function (req, res) {
        const {gtin} = req.params;
        const {domain, subdomain} = getDomainAndSubdomain(req);
    ...
    server.put("/integration/product/:gtin", async function (req, res) {
        const {gtin} = req.params;
        const {domain, subdomain} = getDomainAndSubdomain(req);
    ...
    server.put("/integration/batch/:gtin/:batchNumber", async function (req, res) {
    ...
    server.post("/integration/batch/:gtin/:batchNumber", async function (req, res) {
        const {gtin, batchNumber} = req.params;
        const {domain, subdomain} = getDomainAndSubdomain(req);
...
//    server.delete("/integration/epi/:gtin/:batchNumber/:language/:epiType", basicPreValidationMiddleware);
    ...
    server.get("/integration/filterAuditLogs", async (req, res) => {
        const urlModule = require('url');
        const urlParts = urlModule.parse(req.url, true);
        const {query, start, sort, number} = urlParts.query;

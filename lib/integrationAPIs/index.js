...
//    server.delete("/integration/epi/:gtin/:batchNumber/:language/:epiType", basicPreValidationMiddleware);
    ...
    server.put("/integration/doMigration", requestBodyJSONMiddleware);
    server.put("/integration/doMigration", async (req, res) => {
        let body;
        try {
    ...
    server.get("/integration/checkIfMigrationIsNeeded", async (req, res) => {
        let migrationNeeded;
        try {
        ...
        res.send(200, migrationNeeded.toString());
    })
}

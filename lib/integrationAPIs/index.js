...
module.exports = function (server) {
    ...
    const createOrUpdateBatchHandler = async function (req, res) {
        ...
        try {
            ...
            if (batchMessage.payload.productCode !== gtin || batchNumber !== batchMessage.payload.batchNumber) {
    ...
    async function batchEpiHandler(req, res) {
        ...
        try {
            ...
            language = _language;
            leafletMessage = JSON.parse(payload);
            if (leafletMessage.payload.productCode !== gtin || batchNumber !== leafletMessage.payload.batchNumber || language !== leafletMessage.payload.language) {
                throw new Error("The data provided in the payload does not match the query parameters.");
            }
        } catch (err) {
            res.send(415, err.message);

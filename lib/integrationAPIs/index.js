const version = 1;
const {requestBodyJSONMiddleware, getIntegrationAPIsAuthorizationMiddleware, ThrottlerMiddleware} = require("./utils/middlewares.js");
...
module.exports = function (server) {
    ...
    const throttlerConfig = server.config["componentsConfig"]["integration-api"]["throttlerConfig"];
    ThrottlerMiddleware(server, throttlerConfig);
...
//    server.delete("/integration/epi/:gtin/:batchNumber/:language/:epiType", basicPreValidationMiddleware);
    ...
    server.put("/integration/batchGroupedMessages/:gtin/:batchNumber", async function (req, res) {
        ...
        try {
            await req.monsterController.digestBatchGroupedMessages(domain, subdomain, gtin, decodeURIComponent(batchNumber), messages, req, res);
        } catch (err) {
            res.send(500);
            return;
        }
    ...
    async function recoverObjectHandler(req, res) {
        ...
        let status;
        try {
            status = await controller.recover(...args, req, res);
        } catch (err) {
            console.error(err);
            res.statusCode = 500;
            res.end();

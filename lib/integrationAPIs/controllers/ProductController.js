...
function ProductController(version){
  ...
  this.addProduct = async function(gtin, productMessage, res){
    let product;
    try{
    ...
    try{
      product = await productFactory.lookupProduct(gtin);
      ...
      product = productFactory.createProduct(gtin, version);

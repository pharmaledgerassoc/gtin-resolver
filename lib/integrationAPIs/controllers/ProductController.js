function ProductController(enclave, version) {
    ...
    this.recover = async function(domain, subdomain, gtin, res){
        let auditId;
        const userId = req.headers["user-id"];
        ...
        let temporaryProduct = productFactory.getInstance(domain, subdomain, gtin, version);
        let productVersion;
        try{
            productVersion = await temporaryProduct.recover();
        }catch(err){
            let auditContext = {
        ...
        if(productVersion){
            let auditContext = {
                reason: `The product with GTIN ${productCode} git recovered as version ${productVersion}`,
                gtin,
                userId,

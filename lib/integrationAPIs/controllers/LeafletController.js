function LeafletController(enclave, version) {
    ...
    this.addEPI = async (domain, subdomain, gtin, batchNumber, language, epiType, leafletMessage, req, res) => {
        ...
        try {
            ...
            let {xmlFileContent, otherFilesContent} = leafletMessage.payload;
            try {
                await targetObject.addEPI(language, epiType, xmlFileContent, otherFilesContent);
    ...
    this.deleteEPI = async (domain, subdomain, productCode, batchNumber, language, epiType, leafletMessage, req, res) => {
        const userId = req.headers["user-id"];
         /*try {
        ...
        try {
            ...
            if (!batchNumber) {
                ...
                targetObject = await batchFactory.lookupBatch(domain, subdomain, productCode, batchNumber);
            }
            ...
            try {
                await targetObject.deleteEPI(language, epiType);
                await targetObject.persist();
            } catch (err) {
                await auditService.auditFail(auditId, operationFailContext);
                res.send(500);

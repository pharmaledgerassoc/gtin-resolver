function BatchController(enclave, version) {
    ...
    this.addBatch = async function (domain, subdomain, gtin, batchNumber, batchMessage, res) {
        ...
        try {
            batch = await batchFactory.lookupBatch(domain, subdomain, gtin, batchNumber);
    ...
    this.listLanguages = async (domain, subdomain, gtin, batchNumber,  res) => {
        try {
            let batch;
            batch = await batchFactory.lookupBatch(domain, subdomain, gtin, batchNumber);
            ...
            const languages = await batch.listLanguages();
            return languages;
        } catch (err) {
            //....
            res.send(500);

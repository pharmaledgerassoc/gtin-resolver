function BatchController(enclave, version) {
    ...
    this.recover = async function(domain, subdomain, gtin, batchNumber, res){
        let auditId;
        const userId = req.headers["user-id"];
        ...
        let temporaryBatch = batchFactory.getInstance(domain, subdomain, gtin, batchNumber, version);
        let batchNewVersion;
        try{
            batchNewVersion = await temporaryBatch.recover();
        }catch(err){
            let auditContext = {

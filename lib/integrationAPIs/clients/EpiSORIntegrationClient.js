...
function EpiSORIntegrationClient(domain, subdomain) {
    ...
    this.getProductEPIs = (gtin, language, type, callback) => {
        _sendRequest(`${getBaseURL()}/epi/${gtin}/${language}/${type}`, 'GET', callback);
    }

    this.getBatchEPIs = (gtin, batchNumber, language, type, callback) => {
        _sendRequest(`${getBaseURL()}/epi/${gtin}/${batchNumber}/${language}/${type}`, 'GET', callback);
    }

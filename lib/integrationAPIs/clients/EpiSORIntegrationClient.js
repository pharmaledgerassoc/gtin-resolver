...
function EpiSORIntegrationClient(domain, subdomain) {
    ...
    this.getImage = (gtin, callback) => {
        const url = `${getBaseURL()}/image/${gtin}?domain=${encodeURIComponent(domain)}&subdomain=${encodeURIComponent(subdomain)}`;
    ...
    this.getBatchEPIs = (gtin, batchNumber, language, epiType, callback) => {
        _sendRequest(`${getBaseURL()}/epi/${gtin}/${encodeURIComponent(batchNumber)}/${language}/${epiType}`, 'GET', callback);
    }
    ...
    this.objectStatus = async (productCode, batchNumber, callback) => {
        ...
        let endpoint = `${systemAPI.getBaseURL()}/objectStatus/${productCode}`;
        if(batchNumber){

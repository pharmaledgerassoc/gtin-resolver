...
function ValidationService() {
    ...
    let getImageNamesFromXml = async function (base64XMLFileContent) {
        return new Promise((resolve, reject) => {
            ...
            parser.onerror = function (err) {
                reject(errMap.newCustomError(errMap.errorTypes.WRONG_XML_FORMAT, "XML file wrong format"));
            };
    ...
    this.validateLeafletMessage = async function validateLeafletMessage(leafletMessage) {
        ...
        let xmlImageNames = await getImageNamesFromXml(base64XMLFileContent);

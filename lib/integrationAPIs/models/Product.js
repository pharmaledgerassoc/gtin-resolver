...
function Product(enclave, domain, subdomain, gtin, version) {
    ...
    instance.getJSONStoragePath = () => {
        return `${MUTABLE_MOUNTING_POINT}/product.epi_v${instance.epiProtocol}.json`;
    }

    instance.getLeafletStoragePath = (type, language) => {
    ...
    instance.getLeafletFilePath = (type, language) => {
    ...
    instance.getGTINSSI = () => {
        return GTIN_SSI.createGTIN_SSI(domain, subdomain, subdomain, subdomain, instance.productCode);
    }
...
//todo: check if we still need for the dsu anchoring before calling fixedURL
    ...
    const getImageMountingPoint =  (fileExtension) => {
        return `${MUTABLE_MOUNTING_POINT}/photo${fileExtension}`;
    }
    ...
    let listLanguages = instance.listLanguages;
    instance.listLanguages = async () => {
        return await listLanguages.call(instance);
    }

...
const fixedUrlUtils = require("../../mappings/utils");
...
function ModelBase(enclave, domain, subdomain, gtin) {
    ...
    this.registerFixedUrl = async (language, epiType) => {
        try {
            let fixedUrlUtils = require("./../../mappings/utils.js");
            await fixedUrlUtils.registerGtinOwnerFixedUrlByDomainAsync(domain, gtin);
            await fixedUrlUtils.registerLeafletFixedUrlByDomainAsync(domain, subdomain, epiType, gtin, language, this.batchNumber, this.expiryDate, this.epiProtocol);
            if (this.batchNumber) {
    ...
    this.recover = async () => {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                let recoveredDSU = await recoverDSU(gtinSSI, recoveryImmutableDSU);
                let recoveredMutableDSU = await recoverDSU(seedSSI, recoveryMutableDSU);

                let jsonInitialziationRequired = false;
                try{
                    let jsonSerialization = await recoveredMutableDSU.readFileAsync(this.getJSONStoragePath());
                }catch(err){
...
ModelBase.prototype.checkStatus = async function () {
    ...
    try {
        ...
        let mutableAnchorId = await mutableKeySSI.getAnchorIdAsync();

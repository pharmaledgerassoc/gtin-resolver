...
function ModelBase(enclave, domain, subdomain, gtin) {
    ...
    this.createCache = async (type, language, leafletType, epiMarket, skipLeaflet = false) => {
        try {
            let fixedUrlUtils = require("./../../mappings/utils.js");
            await fixedUrlUtils.deactivateLeafletFixedUrlAsync(undefined, domain, this.productCode, this.batchNumber, language, leafletType, epiMarket);
            await fixedUrlUtils.activateGtinOwnerFixedUrl(undefined, domain, this.productCode, this.batchNumber, language, leafletType, epiMarket);
            await fixedUrlUtils.activateLeafletFixedUrl(undefined, domain, this.productCode, this.batchNumber, language, leafletType, epiMarket);
        } catch (err) {
            //if cleanup fails mapping needs to fail...
            console.log("Failed to trigger FixedUrl", err);
            // const errMap = require("opendsu").loadApi("m2dsu").getErrorsMap();

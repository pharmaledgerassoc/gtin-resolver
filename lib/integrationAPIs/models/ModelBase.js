...
function ModelBase(enclave, domain, subdomain, gtin) {
    ...
    this.addEPI = async (leafletMessage) => {
        ...
        let {language, action, otherFilesContent, xmlFileContent} = leafletMessage.payload;
    ...
    this.getEpi = async (language, type) => {
        const epiPath = this.getLeafletStoragePath(type, language)
        ...
        if (files && files.length > 0) {
            ...
            for (let file of files) {
                ...
                if (file.endsWith("xml")) {
                    epiResult.xmlFileContent = fileContent;
                } else {
                    epiResult.otherFilesContent.push({fileName: file, fileContent: fileContent})
                }
            }
        }

...
function ModelBase(enclave, domain, subdomain, gtin) {
    ...
    this.createCache = async (batchNumber, language, leafletType, epiMarket) => {
        try {
            let fixedUrlUtils = require("./../../mappings/utils.js");
            await fixedUrlUtils.deactivateLeafletFixedUrlAsync(undefined, domain, gtin, batchNumber, language, leafletType, epiMarket);
            await fixedUrlUtils.activateGtinOwnerFixedUrl(undefined, domain, gtin, batchNumber, language, leafletType, epiMarket);
            await fixedUrlUtils.activateLeafletFixedUrl(undefined, domain, gtin, batchNumber, language, leafletType, epiMarket);
        } catch (err) {
            //if cleanup fails mapping needs to fail...
            console.log("Failed to trigger FixedUrl", err);
            // const errMap = require("opendsu").loadApi("m2dsu").getErrorsMap();

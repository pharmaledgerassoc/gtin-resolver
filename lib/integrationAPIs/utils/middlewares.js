...
function RequestLimiter(maxConcurrentRequests, requestTimeout = 30000) {
...
const getRequestLimiterMiddleware = (server) => {
    let config = server.config["componentsConfig"]["integration-api"]["throttlerConfig"]
    if (!config) {
        config = {}
    }
    config.metadataCapacity = config.metadataCapacity || 30;
    config.epiCapacity = config.epiCapacity || 10;
    ...
    server.post("/integration/audit/*", metadataRequestLimiter.requestMiddleware);

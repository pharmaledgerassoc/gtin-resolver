function LightDBEnclaveFactory() {
    ...
    const addEnclaveInstanceInCache = (domain, subdomain, enclaveInstance) => {
        instances[generateEnclaveName(domain, subdomain)] = enclaveInstance;
    }

    const getEnclaveInstanceFromCache = (domain, subdomain) => {
        return instances[generateEnclaveName(domain, subdomain)];
    }
    ...
    this.createLightDBEnclaveAsync = async (domain, subdomain) => {
        ...
        let secret;
        try {
            secret = await secretsServiceInstance.readServerSecretAsync(generateEnclaveName(domain, subdomain));
        } catch (e) {

        }

        if (!secret) {
            throw new Error(`Secret for enclave ${generateEnclaveName(domain, subdomain)} not found`);
        }

        const slots = secret.split(";");
        const enclave = require("opendsu").loadAPI("enclave");
        const lightDBEnclaveInstance = enclave.initialiseLightDBEnclave(generateEnclaveName(domain, subdomain), slots);

...
function getEPIMappingEngineMessageResults(server) {
  ...
  function getLogs(msgParam, domain, callback) {
    ...
    const LOGS_FILE = path.join(LOGS_FOLDER, "logs");
    let result;
    ...
    */
      result = fs.readFileSync(LOGS_FILE, 'utf8');
  ...
  server.put("/mappingEngine/:domain/:subdomain/saveResult", function (request, response) {
    ...
    request.on('end', async () => {
      ...
      try {
        ...
        const logsFile = path.join(fileDir, "logs");
        if (!fs.existsSync(fileDir)) {
          fs.mkdirSync(fileDir, {recursive: true});
        }
        //create file if not exists;
        fs.closeSync(fs.openSync(logsFile, 'a'));

        fs.appendFileSync(logsFile, body + MESSAGE_SEPARATOR);

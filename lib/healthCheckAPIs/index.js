...
function healthCheckAPIs(server) {
    ...
    server.get('/maintenance/installInfo', function (req, res) {
        ...
        let installInfoPk = getInstallInfoPk(healthCheckRunId);
        let result = req.systemHealthController.getInstallInfo();
        lightDBEnclaveClient.insertRecord($$.SYSTEM_IDENTIFIER, healthCheckRunId, installInfoPk, {data: result}, (err) => {
            if (err) {
    ...
    server.get('/maintenance/systemHealth', function (req, res) {
        const urlParts = urlModule.parse(req.url, true);
        const {healthCheckRunId} = urlParts.query;
        let systemHealthPk = getSystemHealthPk(healthCheckRunId);
        let result = req.systemHealthController.getSystemHealthInfo();
        lightDBEnclaveClient.insertRecord($$.SYSTEM_IDENTIFIER, healthCheckRunId, systemHealthPk, {data: result}, (err) => {
            if (err) {
                res.send(500, err.message);
                return;
            }
    ...
    server.get('/maintenance/checkSecrets', function (req, res) {
        try {
            ...
            let result = req.systemHealthController.checkSecrets();
    ...
    server.get('/maintenance/configsInfo', function (req, res) {
        ...
        let configsPK = getConfigsPk(healthCheckRunId);
        lightDBEnclaveClient.insertRecord($$.SYSTEM_IDENTIFIER, healthCheckRunId, configsPK, {data: data}, (err) => {
            if (err) {
                res.send(500, err.message);
                return;
            }
    ...
    server.get('/maintenance/checkWallets', function (req, res) {
        ...
        let walletsPK = getWalletsPk(healthCheckRunId);
        lightDBEnclaveClient.insertRecord($$.SYSTEM_IDENTIFIER, healthCheckRunId, walletsPK, {data: data}, (err) => {
            if (err) {
                res.send(500, err.message);
                return;
            }
    ...
    server.post('/maintenance/healthCheck/run', async function (req, res) {
        ...
        let promises = [];
        promises.push($$.promisify(req.client.checkSecrets)());
        promises.push($$.promisify(req.client.checkInstallInfo)());
        promises.push($$.promisify(req.client.checkSystemHealth)());
        promises.push($$.promisify(req.client.checkConfigsInfo)());
        promises.push($$.promisify(req.client.checkWallets)());
    ...
    server.get('/maintenance/markIterationCompletion', function (req, res) {
        const urlParts = urlModule.parse(req.url, true);
        const {healthCheckRunId, status, failedChecksNr} = urlParts.query;
        lightDBEnclaveClient.getRecord($$.SYSTEM_IDENTIFIER, HEALTH_CHECK_TABLE, healthCheckRunId, (err, record) => {
            if (err) {
                res.send(500, "Failed to get record");
                return;
            ...
            lightDBEnclaveClient.updateRecord($$.SYSTEM_IDENTIFIER, HEALTH_CHECK_TABLE, healthCheckRunId, record, (err) => {
                if (err) {
                    res.send(500, "Failed to update record");
                    return;
                }
                res.send(200, "Success");
            });
    ...
    function getWalletsPk(healthCheckRunId) {
        return healthCheckRunId + "_wallets";
    }
    function getConfigsPk(healthCheckRunId) {
        return healthCheckRunId + "_configsInfo";
    }
    function getSecretsPk(healthCheckRunId) {
        return healthCheckRunId + "_secrets";
    }
    function getInstallInfoPk (healthCheckRunId) {
        return healthCheckRunId + "_installInfo";
    }
    function getSystemHealthPk (healthCheckRunId) {
        return healthCheckRunId + "_systemHealth";
    }

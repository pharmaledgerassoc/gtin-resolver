...
assert.callback("Anchoring Check Test", async (callback) => {
    ...
    let checkId = await apiClient.checkBricking();
    console.log(checkId);
    let response = await apiClient.getCheckStatus(component, checkId);
    assert.true(response.status === constants.HEALTH_CHECK_STATUSES.IN_PROGRESS, "Bricking check should be in progress");
    while(response.status === constants.HEALTH_CHECK_STATUSES.IN_PROGRESS){
        response = await apiClient.getCheckStatus(component, checkId);

...
assert.callback("Secrets Check Test", async (callback) => {
    ...
    assert.true(response.report.corruptSecrets.fixable.length === 1, "Failed to detect the corrupt secret");
    assert.true(response.report.corruptSecrets.fixable[0].secretsContainer === secretsContainer, "Failed to detect the corrupt secret");
    callback();
}, 10000);
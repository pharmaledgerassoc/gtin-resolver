...
assert.callback("Anchoring Check Test", async (callback) => {
    ...
    await apiClient.generateFailure(component, action, args);
    response = await apiClient.checkAnchoring();
    console.log(response);
    assert.true(response.length === 1, "Failed to detect corrupted anchor");
    assert.true(response[0].corruptAnchors.notFixable.length === 1, "Failed to detect corrupted anchor");
    console.log(response[0].corruptAnchors.notFixable[0].anchor);
    assert.true(response[0].corruptAnchors.notFixable[0].anchor === anchorToCorrupt, "Failed to detect corrupted anchor");
    callback();
}, 10000);
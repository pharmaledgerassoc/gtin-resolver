...
function StatusController(server) {
    ...
    this.getCheckStatus = async (req, res) => {
        const {checkType, healthCheckId} = req.params;
        const record = await $$.promisify(lightDBEnclaveClient.getRecord)($$.SYSTEM_IDENTIFIER, checkType, healthCheckId);
        if (record) {
            res.statusCode = 200;
    ...
    this.startProcess = async (checkType, healthCheckId, args) => {
        switch (checkType) {
            case Tasks.ANCHORING:
                if (!args.domain) {
                    await markNeverExecutedCheck(Tasks.ANCHORING, healthCheckId);
            ...
            case Tasks.BRICKING:
                if (!args.domain) {
                    await markNeverExecutedCheck(Tasks.BRICKING, healthCheckId);
                    throw new Error("Domain is required.");
                }
            ...
            case Tasks.DATABASES:
                if (!args.tables) {
                    await markNeverExecutedCheck(Tasks.DATABASES, healthCheckId);

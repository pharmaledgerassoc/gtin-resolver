...
async function processVideoMessage(message) {
  ...
  try {
    if (message.videos.batch) {
      ...
      let logData = await this.mappingLogService.logSuccessAction(message, this.batch, true, null, diffs,batchDSU);
      ...
      let logData = await this.mappingLogService.logSuccessAction(message, this.product, true, null, diffs, productDSU);

...
const {buildQueryParams, createURLQuery } = require("../utils/buildQueryParams");
...
function getActivateRelatedFixedURLHandler(getReplicasFnc){
  return function activateRelatedFixedUrl(dsu, domain, gtin, batchNumber, lang, leafletType, epiMarket, callback){
    ...
    let next = async ()=>{
      ...
      getReplicasFnc(domain, function(err, replicas){
        ...
        const query = createURLQuery(gtin, batchNumber, lang, leafletType, epiMarket);
...
function getDeactivateRelatedFixedURLHandler(getReplicasFnc){
  return function deactivateRelatedFixedUrl(dsu, domain, gtin, batchNumber, lang, leafletType, epiMarket, callback){
    getReplicasFnc(domain, function(err, replicas){
      if(replicas.length === 0){
...
let expose = {
  ...
  deactivateLeafletFixedUrl: $$.promisify(getDeactivateRelatedFixedURLHandler(getReplicasAsSmartUrls)),
  activateGtinOwnerFixedUrl: $$.promisify(getActivateRelatedFixedURLHandler(getReplicasAsSmartUrls)),
  deactivateGtinOwnerFixedUrl: $$.promisify(getDeactivateRelatedFixedURLHandler(getReplicasAsSmartUrls))
}

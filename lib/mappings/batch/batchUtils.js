...
async function getBatchDSU(message, productCode, batchId, create = false) {
  ...
  let productDSUObj;
  try {
      productDSUObj = await productUtils.getProductDSU.bind(this)(message, productCode);
  } catch (e) {
    err = e;
  }
  ...
  if (!productDSUObj) {
    ...
    await this.mappingLogService.logFailedMapping(message, "lookup", `${errorMessage}`);
    throw errMap.newCustomError(errMap.errorTypes.PRODUCT_DSU_LOAD_FAIL, "productCode");
  }

  const {constDSU, productDSU, productMetadata} = productDSUObj;

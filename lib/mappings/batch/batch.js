...
*  */
  ...
  if (typeof this.batch === "undefined") {
    ...
    await $$.promisify(utils.deactivateGtinOwnerFixedUrl)(batchDSU, productCode);
    await $$.promisify(utils.deactivateLeafletFixedUrl)(batchDSU, productCode);
  }
  ...
  try{
    const fixedUrl = require("./../utils");
    await $$.promisify(fixedUrl.registerGtinOwnerFixedUrl)(batchDSU, productCode);
    const leafletUtils = require("./../leaflet/leafletUtils");
    ...
    for(let type of leafletTypes){
      ...
      for(let lang of languages){
        ...
        await $$.promisify(fixedUrl.registerLeafletFixedUrl)(batchDSU, type, productCode, lang, this.batch.batchNumber, expirationDate, this.batch.epiLeafletVersion);
      }
    }

    $$.promisify(fixedUrl.activateGtinOwnerFixedUrl)(batchDSU, productCode);
    $$.promisify(fixedUrl.activateLeafletFixedUrl)(batchDSU, productCode);
  }catch(err){

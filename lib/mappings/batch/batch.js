...
async function processBatchMessage(message) {
  ...
  const {
    ...
    alreadyExists,
    gtinSSI
  } = await batchUtils.getBatchDSU.bind(this)(message, productCode, batchId, true);
...
*  */
  ...
  if (!alreadyExists) {
    try {
      await $$.promisify(batchConstDSU.mount)(constants.BATCH_DSU_MOUNT_POINT, this.batch.keySSI);
      let prodDSU = await this.loadDSU(productMetadata.keySSI);
      let prodKeySSI = await prodDSU.getKeySSIAsString();
      await $$.promisify(batchConstDSU.mount)(constants.PRODUCT_DSU_MOUNT_POINT, prodKeySSI);
    } catch (e) {
      const errUtils = require("../errors/errorUtils");
      errUtils.addMappingError("DSU_MOUNT_FAIL");
      const errMap = require("opendsu").loadApi("m2dsu").getErrorsMap();
      await this.mappingLogService.logFailAction(message, null, "DSU mount fail");
      throw errMap.newCustomError(errMap.errorTypes.DSU_MOUNT_FAIL, "productCode");
    }
  }
  ...
  let logData = await this.mappingLogService.logSuccessAction(message, batchClone, alreadyExists);

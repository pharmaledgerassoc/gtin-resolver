...
*  */
  ...
  if (typeof this.batch === "undefined") {
    ...
    await $$.promisify(utils.deactivateGtinOwnerFixedUrl)(batchConstDSU, productCode);
    await $$.promisify(utils.deactivateLeafletFixedUrl)(batchConstDSU, productCode);
  }
  ...
  try{
    const fixedUrl = require("./../utils");
    await $$.promisify(fixedUrl.registerGtinOwnerFixedUrlByDomain)(this.options.holderInfo.domain, productCode);
    const leafletUtils = require("./../leaflet/leafletUtils");
    ...
    for(let type of leafletTypes){
      ...
      for(let lang of languages){
        ...
        await $$.promisify(fixedUrl.registerLeafletFixedUrl)(batchConstDSU, type, productCode, lang, this.batch.batchNumber, expirationDate, this.batch.epiLeafletVersion);
      }
    }

    $$.promisify(fixedUrl.activateGtinOwnerFixedUrl)(batchConstDSU, productCode);
    $$.promisify(fixedUrl.activateLeafletFixedUrl)(batchConstDSU, productCode);
  }catch(err){

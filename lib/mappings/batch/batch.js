...
*  */
  ...
  this.batch.version = await require("./../utils").getNextVersion(batchDSU.getCreationSSI());
  ...
  if(alreadyExists){
    ...
    batchMetadata = this.batch;
    batchMetadata.pk = require("./../../utils/CommonUtils").getBatchMetadataPK(productCode, batchId);
  ...
  try{
    ...
    for(let type of leafletTypes){
      ...
      for(let lang of languages){
        ...
        await $$.promisify(fixedUrl.registerLeafletFixedUrlByDomain)(...args);
    ...
    $$.promisify(fixedUrl.activateGtinOwnerFixedUrl)(batchConstDSU, anchoringDomain, productCode);
    $$.promisify(fixedUrl.activateLeafletFixedUrl)(batchConstDSU, brickingDomain, productCode);
  }catch(err){

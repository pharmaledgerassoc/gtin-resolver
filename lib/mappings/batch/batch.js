...
*  */
  ...
  let ssi = keyssi.parse(productMetadata.keySSI);
  this.batch.product = await $$.promisify(ssi.getDerivedType)("sread");
  this.batch.product = this.batch.product.getIdentifier();
  ...
  if (!alreadyExists) {
    ...
    let prodKeySSI = await prodDSU.getKeySSIAsString("sread");

...
async function processProductPhotoMessage(message) {
  ...
  try {
    ...
    if(this.product.version === 0){
      //trying to prevent 0 version in case of new DSUs that are not anchored yet because of the batch mode
    ...
    const dbUtils = require("../../utils/DBUtils");
    await dbUtils.createOrUpdateRecord(this.storageService, {pk: this.product.gtin, table:constants.PRODUCTS_TABLE}, this.product);

    let originalCommit = productDSU.commitBatch;
    productDSU.commitBatch = (onConflict, callback) => {
      if (typeof callback === 'undefined') {
        callback = onConflict;
        onConflict = undefined;
      }
      originalCommit.call(productDSU, onConflict, async (err)=>{
        if(!err){

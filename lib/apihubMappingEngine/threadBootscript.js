...
function updateMessagesWithError(messages, err) {
  let messagesWithErrors = messages.map(msg => {
    return {message: msg, reason: err.message, error: errMap.newCustomError(errMap.errorTypes.UNKNOWN, [err])}
  })
...
parentPort.on("message", (taskData) => {
  ...
  return ServerDSUStorageImpl.enableDirectAccess(async (err) => {
    ...
    if (err) {
      ...
      parentPort.postMessage({walletSSI, undigestedMessages: updateMessagesWithError(groupMessages, err)});
      return
    }
    ...
    try {
      ...
      return parentPort.postMessage({walletSSI, undigestedMessages: updateMessagesWithError(groupMessages, e)});
    }
...
process.on("uncaughtException", (error) => {
  ...
  return parentPort.postMessage({walletSSI, undigestedMessages: updateMessagesWithError(threadGroupMessages, error)});
});

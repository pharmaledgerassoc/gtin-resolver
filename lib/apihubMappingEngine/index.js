  ...
  async function storeUndigestedMessages(mappingEnginResultURL, groupMessages, walletSSI, response) {
    ...
    try {
      messagesToPersist.forEach(item => {
        let index = undigestedMessages.findIndex(uMsg => JSON.parse(uMsg).message.messageId === item.requestMessageId)

        if (index >= 0) {
          let undigestedMessage = JSON.parse(undigestedMessages[index]);
  ...
  const processWalletMessages = (walletSSI, holderInfo, mappingEnginResultURL, callback) => {
    ...
    getMessagesFromDb(walletSSI, async (err, messages) => {
      ...
      messagePipe.onNewGroup(async (groupMessages) => {
        ...
        try {
          const response = await dispatchMessagesToWorker(walletSSI, holderInfo, groupMessages);
          await deleteProcessedMessagesFromDb(walletSSI, groupMessages);
          await storeUndigestedMessages(mappingEnginResultURL, groupMessages, walletSSI, response);
        } catch (e) {
          return callback(e);
        }

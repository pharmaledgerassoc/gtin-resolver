...
function getEPIMappingEngineForAPIHUB(server) {
  ...
  const gtinResolverUtils = require("gtin-resolver");
  let LogService = gtinResolverUtils.loadApi("services").LogService;
  const mappings = gtinResolverUtils.loadApi("mappings")
  const MessagesPipe = gtinResolverUtils.getMessagesPipe();
  ...
  async function getMessagePipe(domain, subdomainName, wltSSI, callback) {
    ...
    */
    ...
    if (messagesPipe[walletSSI] === undefined) {
      ...
      const ServerDSUStorageImpl = gtinResolverUtils.loadApi("services").DSUStorage.getInstance(walletSSI);
      try {
